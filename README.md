...
- **本地同步**：支持导入/导出配置文件，与外部 JSON 文件保持同步
- **远程同步**：从远程 URL 同步环境配置，支持 HTTP/HTTPS 协议
...
#### 远程同步
- 在设置页面"远程配置同步"中输入远程配置 URL（支持 http/https）
- 点击"同步"按钮从远程获取环境配置
- 已存在的环境会更新，新环境会自动添加
- URL 会自动保存，下次打开应用可快速再次同步
- 远程配置文件需包含 `environments` 数组字段
